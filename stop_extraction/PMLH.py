...
class PMLHStopDetection:
    ...
    def point_indexes(self, i, j):
        return [self.point_index(p) for p in self.traj_points[i:j]]
    ...
    def all_distances(self, i, j):  
        return [self.distance(point_index_1, point_index_2) for point_index_1, point_index_2 
                        in combinations(self.point_indexes(i, j), 2)] if j-i >=2 else [0]
    ...
    def max_j_deameter(self, i):
        print('max_j_deameter')
        counter = i
        while(counter <= len(self.traj_points) -1):
            print(str(counter))
    ...
    def extract_stays(self):
        stays = []
        i=0
        while(i< len(self.traj_points)):
            print(str(i))
            j_star = self.j_star_candidate(i)
            if self.deameter(i, j_star) > self.roaming_distance_delta:
                i+=1
            else:
                j_star = self.max_j_deameter(i)
                if j_star >= len(self.traj_points):
                    j_star = len(self.traj_points) - 1
